...
## Project Overview
A smart contract that manages student registration, Edit existing student data, and also display the student details for a particular student , using their specific  address (as an ID). </br>
This smart contract allows managing a registry of students only by authorised admistrator.
...
## 📋Contract Details
### Functions:
* ****`constructor()`****:  Function that sets the owner of the contract/admistrator.
...
5. ****editStudent()****: You can edit the personal details if it already exist (and it can be done only by an admistrator or owner authorized) using student intended new details to be updated , like:, `"_newName"`, `"_newAge"` and `"_newCourse"`
...
* ****Gas Optimization:****  Uses mappings for efficient lookups.
